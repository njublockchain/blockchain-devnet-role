#仅对event_type进行编码和Attention权重分配
import torch
...
class RGCNLayerWithAttention(nn.Module):
    def __init__(self, in_channels, out_channels, num_relations):
        super(RGCNLayerWithAttention, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.num_relations = num_relations
        self.attention = nn.Parameter(torch.Tensor(num_relations, 1))
        nn.init.xavier_uniform_(self.attention)
        self.weight = nn.Parameter(torch.Tensor(num_relations, in_channels, out_channels))
        nn.init.xavier_uniform_(self.weight)
        
    def forward(self, x, edge_index, edge_type):
        attention_scores = F.softmax(self.attention, dim=0)
        ...
        for rel in range(self.num_relations):
            ...
            # 选择出当前关系类型的边连接的节点特征
            source_nodes = edge_index_rel[0]  # 源节点索引
            target_nodes = edge_index_rel[1]  # 目标节点索引

            # 对于每种类型的边，使用转换后的特征和注意力得分进行加权
            x_transformed = torch.matmul(x, self.weight[rel])
            for i, (src, tar) in enumerate(zip(source_nodes, target_nodes)):
                out[tar] += attention_scores[rel] * x_transformed[src]

        return out

class RGCNWithAttention(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_relations):
        super(RGCNWithAttention, self).__init__()
        self.num_relations = num_relations  # Store the number of relations
        self.conv1 = RGCNLayerWithAttention(in_channels, hidden_channels, num_relations)
        self.conv2 = RGCNLayerWithAttention(hidden_channels, out_channels, num_relations)
...
# 导出DataFrame到CSV文件
csv_file_path = '/home/ta/devnet/rgcnatt_node_features_matrix.csv'
...
csv_file_path = '/home/ta/devnet/rgcnatt_normalized_features_matrix.csv'
...
#---------------------------------
model = RGCNWithAttention(data.num_features, 128, 32, num_relations).to(device)
...
new_gexf_path = '/home/lab0/devnet/rebuilt/bips_202301_time_price_att_clustered.gexf'
