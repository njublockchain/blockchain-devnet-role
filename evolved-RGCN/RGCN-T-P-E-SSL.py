...
def compute_ssl_loss(embeddings, labels):
    unique_labels = torch.unique(labels)
    loss_within = 0.0
    loss_between = 0.0

    # Iterate over all unique labels to calculate within and between cluster loss
    for i in unique_labels:
        indices_i = (labels == i).nonzero().squeeze(1)
        for j in unique_labels:
            indices_j = (labels == j).nonzero().squeeze(1)

            if i == j:  # Within cluster loss
                embeddings_i = embeddings[indices_i]
                distances = torch.norm(embeddings_i.unsqueeze(0) - embeddings_i.unsqueeze(1), p=2, dim=2)
                loss_within += distances.sum() / (len(indices_i) * (len(indices_i) - 1)) if len(indices_i) > 1 else 0
            else:  # Between cluster loss
                embeddings_i = embeddings[indices_i]
                embeddings_j = embeddings[indices_j]
                distances = torch.norm(embeddings_i.unsqueeze(0) - embeddings_j.unsqueeze(1), p=2, dim=2)
                loss_between += distances.sum() / (len(indices_i) * len(indices_j))

    # Normalize the loss
    ssl_loss = loss_within - loss_between
    return ssl_loss

def train_with_ssl(model, data, optimizer, epochs=100, gamma=1.0):
    for epoch in range(epochs):
        model.train()
        optimizer.zero_grad()
        ...
        # Compute the adjacency matrix within the training function
        adj_matrix = torch.zeros((data.num_nodes, data.num_nodes), device=data.x.device)
        edge_index = data.edge_index.cpu().numpy()
        adj_matrix[edge_index[0], edge_index[1]] = 1
        adj_matrix = adj_matrix.to(data.x.device)

        pred_adj_matrix = torch.sigmoid(torch.mm(embeddings, embeddings.t()))
        loss_rgcn = F.binary_cross_entropy(pred_adj_matrix, adj_matrix)

        # Use KMeans to obtain cluster labels for computing SSL loss
        kmeans = KMeans(n_clusters=5, random_state=42).fit(embeddings.detach().cpu().numpy())
        labels = torch.tensor(kmeans.labels_, dtype=torch.long, device=data.x.device)

        loss_ssl = compute_ssl_loss(embeddings, labels)
        total_loss = loss_rgcn + gamma * loss_ssl

        total_loss.backward()
        optimizer.step()

        if epoch % 10 == 0:
            print(f'Epoch: {epoch}, Loss RGCN: {loss_rgcn.item()}, Loss SSL: {loss_ssl.item()}, Total Loss: {total_loss.item()}')
...
gexf_path = '/home/ta/devnet/data_source/bitcoin_2023_monthly_time_price_gexf/2023-12.gexf'
...
# 导出DataFrame到CSV文件
csv_file_path = '/home/ta/devnet/evolved-RGCN/node_feature_matrix/rgcn_tpe_bitcoin_202312_node_features_matrix.csv'
...
csv_file_path = '/home/ta/devnet/evolved-RGCN/node_feature_normalized_matrix/rgcn_tpe_bitcoin_202312_normalized_features_matrix.csv'
...
train_with_ssl(model, data, optimizer, epochs=20)
...
new_gexf_path = '/home/ta/devnet/bips_tpe_202312_time_price_clustered.gexf'
